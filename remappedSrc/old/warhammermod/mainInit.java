...
*/
        ServerPlayNetworking.registerGlobalReceiver(emptyload.ID,(payload, context) -> {
            ...
            LootTable lootTable = world.getServer().reloadableRegistries().getLootTable(WHRegistry.GIVE_TUTORIAL_BOOK);
            LootParams lootContextParameterSet = new LootParams.Builder(world).withParameter(LootContextParams.ORIGIN, player.position()).withParameter(LootContextParams.THIS_ENTITY, player).create(LootContextParamSets.GIFT);
            ObjectArrayList<ItemStack> list2 = lootTable.getRandomItems(lootContextParameterSet);
            for (ItemStack itemStack : list2) {
    ...
    public static final ResourceLocation HIGHLIGHT_PACKET_ID = ResourceLocation.fromNamespaceAndPath(reference.modid, "shooting_packet");
